...
### Sections
 ...
 10. Program Calls and System Calls
...
Local variables are prefixed with the symbol `%`. They hold a value that can be used as argument of instructions and only accessible by other instructions in the same definition.
```psilang
main {
...
Global variables and constants are prefixed with the symbol `@`. They hold a global value that can be used as argument of instructions and they are usable across all definitions.

Global variables are created using `global` keyword.
...
Global constants are created using `constant` keyword.
example:
```psilang
constant @x 12;
```

Global Variables and Constants name cannot begin with numbers:
```psilang
// valid
@a
@hello_var2
...
### Program Calls and System Calls

#### Program Calls

In PSI we can call functions using `call` instruction:
```psilang
call <function label> <return value> <arg1> <arg2> ... <argN>;
```

example:
```psilang
entry main;
function $int test_func $int $int; // this function can take variable amount of $int at end
...
main {
    call test_func %result 2 3; // return value of function will be stored in %result
}
```

#### System Calls

Since PSI requires an backend, sometimes we need to able to talk to backend. To do that we need to call the System Functions using `sys` instruction.  
```psilang
sys <system function> <return value> <arg1> <arg2> ... <argN>;
```

#### System Functions

System functions are backend defined functions that tells backend to do something. System functions are prefixed with `#` symbol.
```psilang
#write
#fork
#exit
...
Sometimes we want to make sure that a PSI program is valid or not, for example checking if a backend supports a specific type, system function.
To do that we must use the `required` instruction. If the backend can't support a required item, then the backend must return an error and quit processing PSI code.
...
required #write; // if backend dosen't support it, backend should return an error
